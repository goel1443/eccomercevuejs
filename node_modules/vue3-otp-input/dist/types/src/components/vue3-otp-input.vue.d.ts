import { PropType } from "vue";
declare const _default: import("vue").DefineComponent<{
    value: {
        type: StringConstructor;
        default: string;
        required: true;
    };
    numInputs: {
        default: number;
    };
    separator: {
        type: StringConstructor;
        default: string;
    };
    inputClasses: {
        type: PropType<string | string[]>;
    };
    conditionalClass: {
        type: PropType<string[]>;
        default: never[];
    };
    inputType: {
        type: PropType<"number" | "tel" | "letter-numeric" | "password">;
        validator: (value: string) => boolean;
    };
    inputmode: {
        type: PropType<"search" | "tel" | "none" | "text" | "url" | "email" | "numeric" | "decimal">;
        validator: (value: string) => boolean;
        default: string;
    };
    shouldAutoFocus: {
        type: BooleanConstructor;
        default: boolean;
    };
    placeholder: {
        type: PropType<string[]>;
        default: never[];
    };
    isDisabled: {
        type: BooleanConstructor;
        default: boolean;
    };
    shouldFocusOrder: {
        type: BooleanConstructor;
        default: boolean;
    };
}, {
    activeInput: import("vue").Ref<number>;
    otp: import("vue").Ref<string[]>;
    oldOtp: import("vue").Ref<string[]>;
    clearInput: () => void;
    handleOnPaste: (event: any) => void | "Invalid pasted data" | "Wait until the user enters the required number of characters";
    handleOnKeyDown: (event: KeyboardEvent, index: number) => void;
    handleOnBlur: () => void;
    changeCodeAtFocus: (value: number | string) => void;
    focusInput: (input: number) => void;
    focusNextInput: () => void;
    focusPrevInput: () => void;
    handleOnFocus: (index: number) => void;
    checkFilledAllInputs: () => void | "Wait until the user enters the required number of characters";
    handleOnChange: (value: number) => void;
    fillInput: (value: string) => void;
}, unknown, {}, {}, import("vue").ComponentOptionsMixin, import("vue").ComponentOptionsMixin, {}, string, import("vue").VNodeProps & import("vue").AllowedComponentProps & import("vue").ComponentCustomProps, Readonly<import("vue").ExtractPropTypes<{
    value: {
        type: StringConstructor;
        default: string;
        required: true;
    };
    numInputs: {
        default: number;
    };
    separator: {
        type: StringConstructor;
        default: string;
    };
    inputClasses: {
        type: PropType<string | string[]>;
    };
    conditionalClass: {
        type: PropType<string[]>;
        default: never[];
    };
    inputType: {
        type: PropType<"number" | "tel" | "letter-numeric" | "password">;
        validator: (value: string) => boolean;
    };
    inputmode: {
        type: PropType<"search" | "tel" | "none" | "text" | "url" | "email" | "numeric" | "decimal">;
        validator: (value: string) => boolean;
        default: string;
    };
    shouldAutoFocus: {
        type: BooleanConstructor;
        default: boolean;
    };
    placeholder: {
        type: PropType<string[]>;
        default: never[];
    };
    isDisabled: {
        type: BooleanConstructor;
        default: boolean;
    };
    shouldFocusOrder: {
        type: BooleanConstructor;
        default: boolean;
    };
}>>, {
    value: string;
    numInputs: number;
    separator: string;
    conditionalClass: string[];
    inputmode: "search" | "tel" | "none" | "text" | "url" | "email" | "numeric" | "decimal";
    shouldAutoFocus: boolean;
    placeholder: string[];
    isDisabled: boolean;
    shouldFocusOrder: boolean;
}>;
export default _default;
